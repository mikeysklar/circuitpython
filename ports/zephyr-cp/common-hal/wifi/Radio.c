...
//
// Zephyr delivers the echo reply on the network RX thread through an ICMP
// handler, not on the thread that called ping(), so the two halves talk
// through this block of state. A pointer to it lives on the calling thread's
// stack and reaches the handler as the ICMP context's user_data, which keeps
// the implementation free of globals.
typedef struct {
...
// Identifier and sequence for the next echo request. The sequence must change
// from one ping to the next: an ICMP echo carries no port number, so the only
// thing distinguishing two back-to-back requests is this field. Without it a
// late reply to the first call would satisfy the second and be reported as the
// second call's round trip time. The identifier is randomized once so that two
// boards pinging the same host do not accept each other's replies.
...
static enum net_verdict ping_reply_handler(struct net_icmp_ctx *ctx,
    ...
    // net_pkt_get_data() deliberately leaves the packet cursor where it found
    // it. That matters here: returning NET_CONTINUE hands the packet to the
    // next handler in the list, which expects to start reading at the echo
    // header exactly as we did.
...
mp_int_t common_hal_wifi_radio_ping(wifi_radio_obj_t *self, mp_obj_t ip_address, mp_float_t timeout) {
    ...
    // A NULL sta_netif here is fine and means "not associated yet": the stack
    // falls back to selecting an interface from the destination address, and
    // fails the request cleanly if it cannot find one. Passing the interface when
    // we do have it also makes the stack drop replies that arrive on a different
    // one.
    //
    // This is the blocking send on purpose, matching Zephyr's own net shell. It
    // allocates the packet with a one second wait, so if the net_buf pool is
    // exhausted this call can block for up to a second before returning -ENOMEM,
    // and a ping can take timeout + 1s to come back. That is expected, not a bug.
    // net_icmp_send_echo_request_no_wait() would fail instantly instead.
    ...
    // -1 is the only failure value the binding recognizes: wifi_radio_ping() in
    // shared-bindings/wifi/Radio.c turns exactly -1 into None and divides
    // anything else by 1000 to get seconds. Returning 0, as the previous stub
    // did, reads as a successful 0 ms round trip.
