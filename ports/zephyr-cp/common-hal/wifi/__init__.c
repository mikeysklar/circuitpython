...
// Registered at DBG so the only gate is the global CONFIG_LOG_MAX_LEVEL
// (2/WRN in prj.conf). The old printk tracing corrupted the raw-REPL
// handshake on the console UART; routing through the log subsystem keeps
// the REPL byte-clean and the traces one config flip away.
LOG_MODULE_REGISTER(cp_wifi, LOG_LEVEL_DBG);
...
// to pick the right security type per AP. The SiWx91x driver needs WPA2 and
// WPA3 requested explicitly and rejects the wrong one with a generic auth
// failure, so guessing is not viable.
