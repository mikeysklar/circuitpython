...
mp_obj_t common_hal_wifi_network_get_authmode(wifi_network_obj_t *self) {
    // The scan result carries Zephyr's wifi_security_type, filled in by the
    // driver's security_convert[] table. The old body was a commented-out
    // ESP-IDF switch, so authmode ALWAYS returned an empty list -- which reads
    // as "open network" to any caller that checks for AUTHMODE_OPEN, and gives
    // connect() nothing to pick a handshake with.
