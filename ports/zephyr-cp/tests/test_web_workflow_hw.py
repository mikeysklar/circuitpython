...
listener-error bug survived upstream since 2023 (PR #7836). This module
runs the verified-working baseline from mikeysklar/circuitpython#10 against
actual hardware.
...
# workaround for the pool exhaustion in mikeysklar/circuitpython#35; with
# NET_MAX_CONN=12 that
...
def test_http_survives_concurrent_ble_gatt():
    """Sustained GATT reads concurrent with HTTP traffic.

    Leftover from mikeysklar/circuitpython#13.
    ...
    gap noted when mikeysklar/circuitpython#13 closed: coexistence was
    measured with BLE sampled, not stressed.
    """
