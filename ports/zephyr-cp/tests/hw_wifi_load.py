...
full sweep printed rather than asserted:
    ...
    python -m tests.hw_wifi_load --host 192.168.0.39
    python -m tests.hw_wifi_load --host 192.168.0.39 --concurrency 8
    python -m tests.hw_wifi_load --host 192.168.0.39 --sequential 200 --spacing 0
...
---------------------------------------------------------------------------
The board's ceiling is CONFIG_NET_MAX_CONN, and it is RATE-sensitive. An
earlier note here claimed 200ms spacing was clean; six-trial sampling says it
is not, and the corrected numbers are:
...
the descriptor limit was never reached. An earlier version of this harness
predicted the opposite; the prediction was wrong and the spacing test is what
settled it.
