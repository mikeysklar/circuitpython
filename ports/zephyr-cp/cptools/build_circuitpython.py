...
async def build_circuitpython():  # noqa: C901
    ...
    # CIRCUITPY-CHANGE: find_mpconfigboard misses zephyr board aliases, so the toml
    # may not be loaded yet. Load it now that the path is resolved, then honour
    # CIRCUITPY_ENABLE_MPY_NATIVE = true from the board config.
    if (
        mpconfigboard_fn is not None
        and mpconfigboard_fn.exists()
        and "CIRCUITPY_ENABLE_MPY_NATIVE" not in mpconfigboard
    ):
        with mpconfigboard_fn.open("rb") as f:
            mpconfigboard.update(tomllib.load(f))
